...
const NewInterviewFormAdmin = ({ users }) => {
    ...
    const canSave = [userId, title, position, keySkills.length].every(Boolean) && !isLoading

    const onSaveInterviewClicked = async (e) => {
        e.preventDefault()
        if (canSave) {
            await addNewInterview({ user: userId, title, position, keySkills, experience, industry, company })
        }
    }
...
export default NewInterviewFormAdmin
